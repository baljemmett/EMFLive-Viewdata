...
my $frame = new TelstarFrame($frame_number, "a", 1);
...
for my $entry (sort by_name grep $_->{name} =~ /$search_re/i || ($_->{mnemonic} || "") =~ /$search_re/i, @$full_list)
{
    my $mnemonic = $entry->{mnemonic} ? " ($entry->{mnemonic})" : "";
    my $entry_text = sprintf("%6s", $entry->{number}) . $mnemonic . "\x1E" . unidecode($entry->{name});
    ...
    my $letter = uc(substr($entry->{name}, 0, 1));
